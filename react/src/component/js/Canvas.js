...
//document.getElementById('root').appendChild(canvasEle)
    ...
    function animations() {
        ...
        for (let i = 0; i < num; i++) {
            ...
            for (let j = i + 1; j < num; j++) {
                if (Math.pow(data[i].x - data[j].x, 2) + Math.pow(data[i].y - data[j].y, 2) < w / 20 * w / 20) {
                    createLine(data[i].x, data[i].y, data[j].x, data[j].y)
                }
            }
        }
        for (let j = 0; j < num; j++) {
            if (Math.pow(clientX - data[j].x, 2) + Math.pow(clientY - data[j].y, 2) < w / 8 * w / 8) {
